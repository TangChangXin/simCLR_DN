...
def 训练模型(网络模型, 优化器, 预训练权重路径, 硬件设备, 命令行参数, 随机图像变换):
    ...
    有标签训练数据集 = datasets.ImageFolder(root=os.path.join(数据集根路径, "Train"), transform=随机图像变换["训练集"])
    # 训练类列表 = 有标签训练数据集.class_to_idx
    ...
    学习率调整器 = torch.optim.lr_scheduler.ReduceLROnPlateau(优化器, mode='min', factor=0.5, patience=30, cooldown=10)
    最高测试准确率 = 0.0
    # 开始训练
    for 当前训练周期 in range(1, 命令行参数.labeled_train_max_epoch + 1):
        ...
        训练循环 = tqdm(enumerate(有标签训练数据), total=len(有标签训练数据), ncols=100, leave=True)
        for 当前批次, (图像数据, 标签) in 训练循环:
            ...
            训练循环.set_description(f"训练周期 [{当前训练周期}/{命令行参数.labeled_train_max_epoch}] 训练损失：{训练损失.detach().item():.5f} 准确率{当前迭代周期训练准确率:.5f}") # 设置进度条描述
            # 训练循环.set_description(f"训练周期 [{当前训练周期}/{命令行参数.labeled_train_max_epoch}]")
            # 训练循环.set_postfix(训练损失 = 训练损失.detach().item(), 准确率 = 当前迭代周期训练准确率)
        ...
        # 下方代码块不反向计算梯度
        with torch.no_grad():
            测试循环 = tqdm(enumerate(有标签测试数据), total=len(有标签测试数据), ncols=100, leave=True)
            for 当前批次, (图像数据, 标签) in 测试循环:
                ...
                测试循环.desc = "测试周期 [{}/{}] 测试损失：{:.5f} 准确率{:.5f}".format(当前训练周期, 命令行参数.labeled_train_max_epoch, 测试损失.detach().item(), 当前迭代周期测试准确率)  # 设置进度条描述
        # 当前迭代周期测试准确率 = 测试正确的总数目 / len(有标签测试数据集)
...
def 有标签训练(命令行参数):
    ...
    分类模型1 = SimCLRModel.有监督simCLRresnet50(3)
...
if __name__ == '__main__':
    ...
    命令行参数解析器.add_argument('--labeled_train_max_epoch', default=500, type=int, help="有标签训练的最大迭代周期")
